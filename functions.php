...
//* Traduzioni del tema e di WooCommerce
include_once( get_stylesheet_directory() . '/lib/translations.php' );

//* Genesis Title Toggle
include_once( get_stylesheet_directory() . '/lib/genesis-title-toggle.php' );
...
function activenet_enqueue_scripts_styles() {
	...
	//* Foglio di stile personalizzato per modifiche 'al volo'
	//wp_enqueue_style( 'custom-css-stylesheet', CHILD_URL . '/css/custom.css', array(), PARENT_THEME_VERSION );

	//* Offside widget area
	wp_enqueue_script( 'slideout', get_stylesheet_directory_uri() . '/js/slideout.min.js', array( 'jquery' ), CHILD_THEME_VERSION, true );
    wp_enqueue_script( 'slideout-init', get_stylesheet_directory_uri() . '/js/slideout-init.js', array( 'slideout' ), CHILD_THEME_VERSION, true );

}

//* Offside widget area
add_action( 'genesis_header', 'custom_hamburger_btn' );
/**
 * Add Hamburger button in .site-header.
 */
function custom_hamburger_btn() { ?>
...
// Register Slideout widget area.
genesis_register_widget_area(
	array(
		'id'          => 'slideout',
		'name'        => __( 'Slideout', 'my-theme-text-domain' ),
		'description' => __( 'Widgets place in this section will appear inside the Slideout side menu', 'my-theme-text-domain' ),
	)
);

add_action( 'genesis_before', 'custom_side_menu' );
/**
 * Add .side-menu above .site-container.
 */
function custom_side_menu() {
	genesis_widget_area( 'slideout', array(
		'before'    => '<div class="side-menu"><div class="wrap">',
		'after'     => '<span class="close-icon dashicons dashicons-no-alt"></span></div></div>',
	) );
}
...
//* Put the secondary navigation menu on top
...
//add_action( 'woocommerce_single_product_summary', 'genesis_do_breadcrumbs',4 ); // Se lo vuoi sopra al nome del prodotto. In quel caso, disattiva quello qui sotto.
